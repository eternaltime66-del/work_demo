...
**全局通用普攻**：`code = DEFAULT_NORMAL`，技能库中唯一、不可删除但可以编辑；角色与怪物没有配置普攻时运行时读取它。
...
- 系统默认普攻模板配置为每经过 1 VA 获得 1 点充能，需求读取单位当前行动值；后台可编辑或创建其他普攻规则；
...
| `BATTLE_START` | 开战立即、指定 AV 一次或周期触发 | `app_skill_output` |
| `BATTLE_JUDGE` | 左右公式持续判定 | `app_skill_output` + Buff |
| `BATTLE_PULSE` | 跨越公式阈值时脉冲触发 | `app_skill_output`；可限 `maxTriggerPerBattle` |
| `BATTLE_COMBAT` | 施法、受击、伤害、击杀、闪避等战斗事件 | `app_skill_output` |

**战斗被动语义对照**

| 类型 | 触发 | 属性增益 |
|------|------|----------|
| 开战 `BATTLE_START` | 开战或指定 AV 时机 | 可输出伤害、治疗、属性或 Buff |
| 判定 `BATTLE_JUDGE` | 公式条件上升沿 | 可配合判定 Buff 维持风格效果 |
| 脉冲 `BATTLE_PULSE` | 阈值阶梯跨越 | 可一次跨越多档并连续结算 |
| 事件 `BATTLE_COMBAT` | 施法/受击/伤害/击杀/闪避 | 可按技能类型、流派、元素或指定技能过滤 |

`ATTR_MODIFY` 属性键含基础（ATK/MAX_HP/DEF）与高级（吸血/攻速/造成·受到伤害比例/最终攻防血）；公式算增益量。高级键接入战斗伤害/吸血/攻速重算。

战斗事件通过 `CombatEventType` 选择触发时机，并可配合技能匹配（任意 / 类型 / 流派 / 元素 / 指定技能）。
...
| 玩家角色 | `app_player_role_skill`；未绑定普攻时运行时读取全局通用普攻，不写冗余绑定 |
...
- 普攻与其他技能一样读取自己的充能配置；默认模板采用每 1 VA +1，并以 `action` 为充能需求。
