...
| 层级 | 表 | ID 前缀 | 关键字段 |
|------|-----|---------|----------|
| 分类 | `app_stage_type` | STY | `name` / `code`（如 MAIN）/ `enable` / `sort` |
| 大关（章） | `app_stage_chapter` | SCP | `typeId` → 分类，`name` / `code` / `sort` / `enable` |
| 小关 | `app_stage_level` | SLV | `chapterId` → 章，`name` / `code`（如 `1-1`）/ `sort` / `enable` |
...
- 普攻（NORMAL）**不会**作为「被充能的技能」去吃 SKILL_CHARGE（只作为触发源）。
- 小技能 / 大招通过充能攒满后优先于普攻出手。
...
- 每个存活单位 `actionProgress +1`；达到自身 `action`（行动阈值）则**出手一次**，进度清零。  
- 同 tick 多单位出手：按阵营 → `posRow` → `posCol` 排序。

### 6.2 出手逻辑

1. `pickSkill`：优先已充满的非普攻；否则普攻。  
2. `castSkill`：扣充能 → 依次结算该技能全部 `SkillEffect` → 写日志与事件。  
3. 若刚放的是充能技能路径 → 触发 **锚点被动**。  
4. 可连锁再放已充满的充能技能（有次数上限，防死循环）。  
5. 状态变化总线触发 **周期被动**（避免伤害类递归）。
...
- `logs`：文本战报  
- `events`：结构化事件（`BATTLE_START` / `TURN_START` / `CAST` / `HIT` / `HEAL` / `CHARGE` / `DEATH` / `BATTLE_END` 等）  
