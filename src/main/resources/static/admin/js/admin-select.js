  ...
  var AdminSelect = {
    ...
    methods: {
      ...
      updatePopover: function () {
        ...
        // 按最长选项估算宽度，避免触发器偏窄时下拉文案被截断
        var maxLabel = 0;
        (this.normalized || []).forEach(function (o) {
          var text = (o.group ? (o.group + ' · ') : '') + String(o.label || '');
          if (text.length > maxLabel) maxLabel = text.length;
        });
        var estimated = Math.min(520, Math.max(rect.width, maxLabel * 14 + 56, 168));
        var left = Math.min(rect.left, Math.max(8, window.innerWidth - estimated - 8));
        this.popover = {
